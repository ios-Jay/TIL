...
/* startFunction() 함수의 실행 메모리 구조
 ...
 - 임시공간 생성(리턴), i(let i = 12) 생성. 결과값( i * i = 12 * 12) 생성
...
/* 정리
 ...
 코드: 앱 실행시, 모든 코드가 일단 코드영역에 올라감. 그리고 순
 한줄씩 실행
 ...
 - main() 함수가 실행이 되고 전역변수가 순차적으로 데이터 영역
 ...
 - addOneMore2() 함수 실행. 스택프레임 형성. 리턴형이 있으
 생성
 ...
 - print(num) 실행. print 함수 이므로 주소를 찾아간뒤 pri
 프레임 형성
